export async function validateTotp(token, secret): Promise<boolean> {
    const timeStep = 30;
    const tolerance = 1;
...
export async function generateToken(timeCounter, secret): Promise<string> {
...
function leftpad(str, len, pad): string {
    if (len + 1 >= str.length) {
